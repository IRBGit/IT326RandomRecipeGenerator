...
from model import User, Recipe, Ingredient, UserSearch, RecipeIngredient, PantryItem, Rating
...
class UserRepository(BaseRepository):
    ...
    def get_by_id(self, user_id: int) -> User | None:
        return (
            ...
            .options(
                ...
                selectinload(User._pantry)
                    .selectinload(PantryItem.ingredient),
                selectinload(User._ratings).selectinload(Rating.recipe),
                selectinload(User._recipe_notes)
            )
    ...
    def get_by_email(self, email: str) -> User | None:
        return (
            self.session.query(User)
            .options(
                selectinload(User._pantry).selectinload(PantryItem.ingredient),
                selectinload(User.favorites),
                selectinload(User._ratings).selectinload(Rating.recipe),
                selectinload(User._recipe_notes)
            )
...
class RecipeRepository(BaseRepository):
    ...
    def get_by_id(self, recipe_id: int) -> Recipe | None:
        return (
            ...
            .options(
                selectinload(Recipe._ingredients)
                    .selectinload(RecipeIngredient.ingredient),
                selectinload(Recipe._ratings),
                selectinload(Recipe._user_notes),
    ...
    def search_by_name(self, recipe_name: str) -> list[Recipe]:
        """
        Returns anything with partial name matches.
        """
        return (
            self.session.query(Recipe)
            .options(
                selectinload(Recipe._ingredients).selectinload(RecipeIngredient.ingredient),
                selectinload(Recipe._ratings),
                selectinload(Recipe._user_notes),
                selectinload(Recipe.favorited_by)
            )
    ...
    def get_by_name(self, recipe_name: str) -> Recipe | None:
        """
        Returns an exact match for the name.
        """
        return (
            self.session.query(Recipe)
            .options(
...
class IngredientRepository(BaseRepository):
    ...
    def get_by_id(self, ingredient_id: int) -> Ingredient | None:
        return (
            ...
            .options(
                selectinload(Ingredient.recipe_associations),
                selectinload(Ingredient.pantry_items)
            )
